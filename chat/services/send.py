...
def send_message_to_participant(participant_id: str, message: str):
    ...
    try:
        ...
        msg = (
            f"Participant {participant_id} not found (404)"
            if status == 404
            else f"Payload too large for participant {participant_id} (413)"
            if status == 413
...
def send_message_to_participant_group(group_id: str, message: str):
    ...
    try:
        ...
        msg = (
            f"Participant group {group_id} not found (404)"
            if status == 404
            else f"Payload too large for participant group {group_id} (413)"
            if status == 413
            else f"HTTP error {status}"
        )
